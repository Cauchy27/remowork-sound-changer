...
実装 → レビュー → 修正のループは**「1ページ / 1機能」単位**（画面 + API 接続 + 対象テスト）でのみ回す。
ファイル追加・関数追加・小修正のたびに回さず、実装単位が**動く状態**に達した時点で1回だけレビューする。
修正は P0/P1 のみ、**再確認は修正差分に限定**（単位全体の再レビューはしない）。P2 は記録して次の単位へ。1単位ごとにコミットする。

Codex プロンプトへの埋め込み: `.claude/docs/codex-prompt-guideline.md`
...
**フロントエンドの状態把握・デバッグ・パフォーマンス分析には chrome-devtools MCP を使用すること。使用方法の詳細は `.claude/docs/chrome-devtools-mcp.md` を参照。**
