...
プロンプトの書き方は [Codex プロンプト設計ガイドライン](codex-prompt-guideline.md) を参照（本ポリシーは経路と役割、あちらは中身）。Codex へ渡す全プロンプトには【モデル運用】ブロック（監督 = Sol / 実作業 = Luna Max / 大問題時のみ Sol・Terra）を必ず含める（省略不可。ブロック原文は同ガイドライン参照）。 なお **Luna の reasoning effort は必ず `max` を明示する**（`gpt-5.6-luna` の既定は `medium` のため、無指定では max にならない）。
